...
var simple_storage = require('../lib/storage/simple_storage.js')
var ConsoleLogger = require('../lib/console_logger.js')
...
function Botkit (configuration) {
  ...
  function Conversation (task, message) {
      ...
      if (this.handler) {
        ...
        if (typeof (this.handler) === 'function') {
          ...
          patterns.forEach(function (pattern) {
            if (pattern && (match = message.text.match(pattern))) {
              message.match = match
              patterns.callback(message, this)
              return
            }
          })
    ...
    this.combineMessages = function (messages) {
      if (messages.length > 1) {
        ...
        last_user = ''
        messages.forEach(function (message) {
          if (multi_users && message.user !== last_user) {
            last_user = message.user
            if (txt.length) {
              txt.push('')
            }
            txt.push('<@' + message.user + '>:')
          }
          txt.push(message.text)
        })
  ...
  botkit.storage = {
    ...
    services: {
      get: function (service, cb) {
        cb(null, botkit.memory_store.services[service])
      },
      save: function (service, cb) {
        botkit.log('Warning: using temporary storage. Data will be lost when process restarts.')
        if (service) {
          botkit.memory_store.services[service.name] = service
          cb(null, service)
        } else {
          cb('No service specified')
        }
      },
      all: function (cb) {
        cb(null, botkit.memory_store.services)
      }
    }
  }
  ...
  botkit.spawn = function (config, cb) {
    var worker = new this.worker(this, config) // eslint-disable-line new-cap
    if (cb) { cb(worker) }
    return worker
  }
  ...
  botkit.tick = function () {
    botkit.tasks.forEach(function (task) {
      task.tick()
    })
  ...
  if (configuration.storage) {
    if (
      ...
      configuration.storage.services.get &&
      configuration.storage.services.save
    ) {
