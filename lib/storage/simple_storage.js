...
module.exports = function (config) {
  ...
  var services_db = new Store(config.path + '/services', {saveId: 'name'})
  ...
  var storage = {
    ...
    services: {
      get: function (service, cb) {
        services_db.get(service, cb)
      },
      save: function (service, cb) {
        services_db.save(service.name, service, cb)
      },
      all: function (cb) {
        services_db.all(objectsToList(cb))
      }
    }
  }
